...
module.exports = Core.extend({
  ...
  initializing: {
    initVars: function() {
      this.appDir = path.join(this.config.get('apps'), this.appName);
    },
    /**
  ...
  creating: {
    ...
    models: {
      src: '_model_item.py',
      dst: '{{app}}/models.py',
      isRun: function(self, dst) {
        return self.fs.exists(dst);
      },
      replacement: function(self, content, dst, src, context) {
    ...
    modelName: {
      ...
      isRun: function(self) {
        return self.fs.exists(f('{{app}}/models', self.context));
      },
      replacement: function(self, content, dst, src, context) {
        var modelCode = f(self.fs.read(src), context);
        return content + modelCode;
      ...
      context: function(self) {
        return {
          fileName: self.model || self.modelName
        };
      }
    },
    ...
    adminName: {
      ...
      isRun: function(self) {
        return self.fs.exists(f('{{app}}/admin', self.context));
      },
      replacement: function(self, content, dst, src, context) {
        var code = f(self.fs.read(src), context);
        return content + code;
      ...
      context: function(self) {
        return {
          fileName: self.admin || self.modelName
        };
      }
    },
    ...
    modelInit: {
      ...
      replacement: function(content, self) {
        var imports = [
          format('from apps.%s.models.%s import *',
            self.appName, self.model || self.modelName.toLowerCase())
        ];
        return self.includeImports(content, imports);
      }
    },
    ...
    adminInit: {
      ...
      replacement: function(content, self) {
        var imports = [
          format('from apps.%s.admin.%s import *',
              self.appName, self.admin || self.modelName.toLowerCase())
        ];
        return self.includeImports(content, imports);
      }
    }
