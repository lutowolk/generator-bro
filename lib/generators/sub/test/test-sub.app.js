    ...
    beforeEach(function (done) {
      ...
      helpers.run(path.join(generatorsDir, 'sub'))
          .inDir(path.join(__dirname, './tmp'), function (dir) {
            ...
            var urls = fs.readFileSync(path.join(
                    __dirname, '../../app/templates/server/config/_urls.py'));

            var installedApps = fs.readFileSync(path.join(
                    __dirname,
                    '../../app/templates/server/config/_installed_apps.py'));
            ...
            fs.writeFileSync(
                'server/config/settings/installed_apps.py', installedApps);
          })
          .withArguments(['blog'])
          .on('end', done);
    });
