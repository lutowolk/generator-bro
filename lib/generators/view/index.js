  ...
  creating: {
    ...
    viewNamed: {
      ...
      context: function(self, globalContext) {
        ...
        if (fs.existsSync(viewsPackage)) {
          fileName = 'views/' + (self.model || self.modelName).toLowerCase();
        }
    ...
    urls: {
      ...
      replacement: function (self, content) {
        ...
        return content.replace(urlpattern, fixUrlpattern);

        function addUrlPattern(view) {
          console.log(view.urlpattern);

          fixUrlpattern = fixUrlpattern.replace(
              /(patterns\(\n?(\s*)'[\w_.]*?',(\s|.)+)\)/,
              format('$1$2%s,\n)', view.urlpattern));
        }
      }
    ...
    templateDetail: {
      ...
      dst: '{{templates}}/{{lower modelName}}_detail.html',
      isRun: function(self, src, dst) {
    ...
    templateForm: {
      ...
      dst: '{{templates}}/{{lower modelName}}_form.html',
      isRun: function(self, src, dst) {
    ...
    templateList: {
      ...
      dst: '{{templates}}/{{lower modelName}}_list.html',
      isRun: function(self, src, dst) {
