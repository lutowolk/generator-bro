  ...
  creating: {
    ...
    viewNamed: {
      ...
      replacement: function(self, content, src, dst, context) {
        ...
        self.log(
          format('%s views were added on to %s', self.viewInserted, dst));

        if (self.viewInserted === 0) {
          process.exit(1);
        }
    ...
    urls: {
      ...
      replacement: function (self, content, src, dst) {
        ...
        var changeContent = content
          .replace(urlpattern, fixUrlpattern.replace('$', '$$$'));
        ...
        function addUrlPattern(view) {
          var isUrlPatternExists = fixUrlpattern
            .indexOf(view.urlpattern.replace('$$', '$')) === -1;
          ...
          function replacer(str, p1, p2) {
            return p1 + p2 + view.urlpattern.replace('$', '$$') + ',\n)';
          }
        }
      }
    },
    ...
    templateDetail: {
      ...
      context: function(self, global) {
        return _.isFunction(self.views.detail.templateContext) ?
            self.views.detail.templateContext() :
            self.views.detail.templateContext;
      }
    },
    ...
    templateForm: {
      ...
      context: function(self, global) {
        return _.isFunction(self.views.create.templateContext) ?
            self.views.create.templateContext() :
            self.views.create.templateContext;
      }
    },
    ...
    templateList: {
      ...
      context: function(self) {
        return _.isFunction(self.views.list.templateContext) ?
            self.views.list.templateContext() :
            self.views.list.templateContext;
      }
    }
  },
